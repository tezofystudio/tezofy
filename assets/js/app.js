  ...
  function pageTemplate(overrideId) {
    ...
    $("#saveBtn")?.addEventListener("click", (e) => {
      ...
      if (list.includes(p.id)) {
        ...
        if ($("#saveBtn span")) $("#saveBtn span").textContent = "Save";
        toast("Removed from saved");
      } else {
        list.push(p.id);
        store.set("saved", list);
        $("#saveBtn")?.classList.add("saved", "on");
        if ($("#saveBtn span")) $("#saveBtn span").textContent = "Saved";
        toast("Saved! Find it in the Saved tab 🔖");
      }
      resetIdleTimer();
    });
    ...
    $("#waBtn")?.addEventListener("click", async (e) => {
      e.stopPropagation();
      const text = await resolveShareUrl(p);
      window.open("https://wa.me/?text=" + encodeURIComponent(text), "_blank", "noopener");
      resetIdleTimer();
    });
    ...
    $$(".strip-thumb-item").forEach((thumb) => {
      thumb.addEventListener("click", () => {
        ...
        if (tid !== p.id) {
          const tIdx = swipeList.findIndex((x) => x && x.id === tid);
    ...
    if (imgEl) {
      ...
      imgEl.addEventListener("touchend", (e) => {
        ...
        if (Math.abs(diffX) > 38 && Math.abs(diffX) > Math.abs(diffY) * 1.3) {
          if (diffX < 0) goToPrompt(nextP.id, 'next');
          else goToPrompt(prevP.id, 'prev');
        }
      }, { passive: true });
    }

    // 💻 ডেস্কটপে কীবোর্ড অ্যারো কী
